<#
.SYNOPSIS
    Converts a GIF into a Flipper Zero-compatible asset pack.

.DESCRIPTION
    This script takes a .gif file and converts it into a Flipper Zero-compatible asset pack,
    which consists of .bm frame files and a meta.txt configuration. It supports a wide range
    of visual and functional customization options, including speech bubbles, dithering, sharpening,
    grayscale conversion, frame rate control, and more.
...
.EXAMPLE
    ...
    Uses default options and prompts user for .gif and destination.
...
.EXAMPLE
    .\gifToFlipperAsset.ps1 -BubbleLocale "bottomright" -BubbleText "Hello\nFlipper!" -StartFrame 0 -EndFrame 10
    Places a speech bubble in the bottom-right corner visible from frame 0 to 10.
